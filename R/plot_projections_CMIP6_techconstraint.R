...
scen_labels <- c('Standard', 'No Technology Penetration Constraint')
...
p_series <- ggplot() + geom_ribbon(data=co2_q, 
  ...
  scale_y_continuous(expression(CO[2]~Emissions~(Gt~CO[2]/yr)), limits=c(0, 120), expand=c(0, 0)) + 
...
pdf('figures/emissions-CMIP6-techpen.pdf', height=4, width=3.5)
grid.draw(fig)
dev.off()

png('figures/emissions-CMIP6-techpen.png', height=4, width=3.5, units='in', res=600)
grid.draw(fig)
dev.off()
