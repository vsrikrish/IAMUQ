...
p_series <- ggplot() + geom_ribbon(data=co2_q, aes(x=Year, ymin=lower, ymax=upper, fill=case), 
                                   color=NA, alpha=0.3) +  
  ...
  scale_linetype_discrete('Marker') + scale_fill_manual('Prior Scenario', values=cbbpsqualitative) + 
  ...
  guides(color = guide_legend(order=1, nrow=3, byrow=FALSE, override_aes=list(size=5)), 
         linetype = guide_legend(order=2), fill = guide_legend(order=3, nrow=2, byrow=FALSE))

p_marg <- ggplot() + 
  stat_density(data=co2_2100, aes(x=value, fill=scenario, color=scenario), alpha=0.3, geom='area', 
               position='identity') + 
  scale_x_continuous(limits=c(0, 40), expand=c(0, 0)) + 
  scale_y_continuous(expand=c(0, 0)) + 
  coord_flip() + 
  scale_fill_manual('', values=cbbpsqualitative) + scale_color_manual('', values=cbbpsqualitative) +
  theme_classic(base_size=5) +
