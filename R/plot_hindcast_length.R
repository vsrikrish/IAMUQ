...
# plot CIs and data
  p_ci[[i]] <- ggplot() + geom_ribbon(data=q[['base']], aes(x=year, ymin=lb, ymax=ub, fill='ci_base'), alpha=0.2) + geom_ribbon(data=q[['short']], aes(x=year, ymin=lb, ymax=ub, fill='ci_short'), alpha=0.2) + geom_point(data=dat[[i]], aes(x=year, y=value, color='point'), size=0.75) + geom_line(data=q[['base']], aes(x=year, y=median, color='median_base')) + geom_line(data=q[['short']], aes(x=year, y=median, color='median_short')) + scale_color_manual('', labels=c('Median (1820-2014)', 'Median (1950-2014)', 'Observation'), values=cols) +  scale_fill_manual('', labels=c('90% Credible Interval (1820-2014)', '90% Credible Interval (1950-2014)'), values=cols) + scale_x_continuous('Year', expand=c(0.001, 0.001), limits=c(1950, 2100)) + scale_y_continuous(ylab[i], expand=c(0.001, 0.001)) + guides(color = guide_legend(override.aes = list(linetype=c(1, 1, NA), shape=c(NA, NA, 16)))) + theme(legend.spacing = unit(-0.15, "cm"), plot.margin=margin(0.2, 0.2, 0, 0.1, unit='in'), legend.box='vertical') + labs(tag=letters[i])
}
...
g <- grid_arrange_shared_legend(p_ci[[1]], p_ci[[2]], p_ci[[3]], ncol=1, nrow=3, position='bottom')

pdf('figures/hindcast_length.pdf', width=4.5, height=8)
grid.draw(g)
dev.off()


png('figures/hindcast_length.png', width=4.5, height=8, units='in', res=600)
grid.draw(g)
dev.off()

for (i in 1:length(p_ci)) {
  p_ci[[i]] <- p_ci[[i]] + theme_classic(base_size=10) + ggtitle(titles[i]) + theme(legend.box='vertical', plot.margin=unit(c(0.2, 0.5, 0, 0.1), 'in'))
}

g <- grid_arrange_shared_legend(p_ci[[1]], p_ci[[2]], p_ci[[3]], ncol=3, nrow=1, position='bottom')

png('figures/hindcast_slide.png', width=8, height=4, units='in', res=600)
grid.draw(g)
dev.off()
