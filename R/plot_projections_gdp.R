...
ssp_melt[, 'value'] <- ssp_melt$value * 1.15 # adjust for inflation between 2005 and 2011

yrs <- 2015:2100

dat <- lapply(iamdata, function(l) {l[l$year %in% 1820:2014,]})
obs <- dat[['prod']][dat[['prod']]$year %in% 2000:2014, ]
...
get_gdp <- function(sim_out, yrs) {
  do.call(cbind, lapply(sim_out, function(l) l$out$Q[l$out$year %in% yrs]))
}
...
gdp <- lapply(sim_out, get_gdp, yrs=yrs)

gdp_q <- lapply(gdp, out_q, yrs=yrs)
gdp_q <- bind_rows(gdp_q, .id='scenario')
gdp_q$case <- factor(gdp_q$scenario, levels=scenarios, labels=scen_labels)

gdp_2100 <- lapply(gdp, out_dist, yr=2100)
gdp_2100 <- bind_rows(gdp_2100, .id='scenario')
gdp_2100$case <- factor(gdp_2100$scenario, levels=scenarios, labels=scen_labels)

p_series <- ggplot() + geom_ribbon(data=gdp_q, 
  ...
  scale_y_continuous('Global Production (trillion 2011$/yr)', limits=c(0, 750), expand=c(0, 0)) + 
...
p_marg <- ggplot() + stat_density(data=gdp_2100, 
...
pdf('figures/gdp-SSP.pdf', height=4, width=3.5)
grid.draw(fig)
dev.off()

png('figures/gdp-SSP.png', height=4, width=3.5, units='in', res=600)
grid.draw(fig)
dev.off()
